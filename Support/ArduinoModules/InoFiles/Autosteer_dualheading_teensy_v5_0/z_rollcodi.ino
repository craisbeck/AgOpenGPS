...
{ // not any more - April 30, 2019 - roll to right is positive Now! Still Important
  ...
  fixnorddeci = strtod(norddeci.c_str(), NULL); // norddeci.toDouble();
  nordWinkel = strtod(nordGrad.c_str(), NULL);
  fixeastdeci = strtod(eastdeci.c_str(), NULL);
  eastWinkel = strtod(eastGrad.c_str(), NULL);
