void NMEA_read() {
  ...
  if ((inByte == 13) && (start == 1)) { // CR > Datensatzende > NMEA ausgeben
    ...
    if (nmea.substring(1, 6) == "GNGGA") {   // Data from GGAm with high and ,
      ...
      GGAZeitNummer = strtod(GGAZeit.c_str(), NULL); //GGAZeit.toDouble();
      ...
      GGASats = strtod(GGASat.c_str(), NULL);
      GGA_hDop = (nmea.substring(jGGA2 + 5, jGGA2 + 9)); //How many hDop;
      //      Serial.println(nmea.substring(jGGA2 + 6, jGGA2 + 7));
      
      if (nmea.substring(jGGA2 + 6, jGGA2 + 7) == ".")   
      {
        GGA_hDops = strtod(GGA_hDop.c_str(), NULL);
      }
      else 
      {
        GGA_hDops = -1;
      ...
      GGA_seahighs = strtod(GGA_seahigh.c_str(), NULL);
      ...
      GPSqualin1 = strtod(GPSquali.c_str(), NULL);
    ...
    */
    ...
    if (nmea.substring(3, 6) == "VTG")  {   // Data from VTG, speed in knoten and VTG Heading
      ...
      if (millis() - prev_PWM_Millis > 100) 
      {
        ...
        // TODO: ledcWriteTone(0, mph_Pwm);
      }
      
      speeed = strtod(VTGspeed.c_str(), NULL);
      VTGheadingnord = (nmea.substring(jVTG3, jVTG4));  // Heading by geographical North
      if (VTGheadingnord == "") headingnord = 0;
      else headingnord = strtod(VTGheadingnord.c_str(), NULL);
